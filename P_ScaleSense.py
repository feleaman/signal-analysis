...
InputsOpt_Defaults = {'mypath':None, 'range':None, 'mother_wavelet':'morl', 'widths':[1,21], 'cmap':'plasma', 'level_contour':10, 'output':'plot', 'level_wavelet':5, 'filter':['highpass', 5.e3, 3], 'mode_stft':('window', 0.035*3), '3d_plot':'mesh', 'window':'hanning'}

def main(argv):
	config = read_parser(argv, Inputs, InputsOpt_Defaults)
	mydir = abspath(getcwd())
	
	if config['mode'] == 'cwt_analysis':
		#+++read data
		...
		lf, lt = np.shape(cwtmatr)
		std_cwt = np.zeros(lt)
		for i in range(lt):
			print('...', i/lt*100)
			# std_cwt[i] = np.std(cwtmatr[:,i])
			std_cwt[i] = kurtosis(cwtmatr[:,i], fisher=True)
		plt.plot(time, std_cwt, 'r')
		plt.show()
		sys.exit()
		
		plt.plot(cwtmatr[9,:])#tiempo
		plt.show()
		plt.plot(cwtmatr[:,9])#freq inv
		plt.show()
	...
	elif config['mode'] == 'stft_analysis':
		from W_Fourier import shortFFT
		#+++read data
		if config['mypath'] == None:
			print('select signal file: ')
			root = Tk()
		...
		lf, lt = np.shape(stftX)
		std_cwt = np.zeros(lt)
		for i in range(lt):
			# print('...', i/lt*100)
			std_cwt[i] = np.std(stftX[:,i])
			# std_cwt[i] = skew(stftX[:,i])
			# plt.plot(stftX[:,i])
			# plt.show()
			# std_cwt[i] = kurtosis(stftX[:,i], fisher=True)
		plt.plot(t_stft, std_cwt, 'r')
		plt.show()
		sys.exit()
		...
		if config['3d_plot'] == 'mesh':
			fig, ax = plt.subplots()
			ax.pcolormesh(t_stft, f_stft, stftX, shading='auto')
			ax.set_xlabel('Time [s]')
			ax.set_ylabel('Frequency [Hz]')
			ax.set_title('STFT')
			...
			tgato, fgato = np.meshgrid(t_stft, f_stft)
